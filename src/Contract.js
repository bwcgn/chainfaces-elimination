...
const api = {
    ...
    ownerCount() {
        console.log();
        if (Object.keys(this.OwnerCount).length > 1) {
            return;
        }

        for (const i in this.Database) {
            if (! this.OwnerCount[ this.Database[i] ]) {
                this.OwnerCount[ this.Database[i] ] = 0;
            }

            this.OwnerCount[ this.Database[i] ] ++
        }
    },
    async getMemorial() {
        let start = this.MemorialStart;

        this.ownerCount();

        let check = this.Eliminated.length === 0;
        ...
        for (const event of tokenTransfers) {
            ...
            let Owner = this.Database[tokenId];
            ...
            if (check) {
                if (!this.DeathCount[Owner]) {
                    this.DeathCount[Owner] = 0;
                }

                this.DeathCount[Owner]++
            }

            if (this.DeathCount[Owner] === this.OwnerCount[Owner]) {
                this.Eliminated.push(Owner);
            }

            // this.Svg[tokenId] = await this.Contract.methods.renderSvg(tokenId).call();
            // console.log(this.Svg[tokenId]);
        }
