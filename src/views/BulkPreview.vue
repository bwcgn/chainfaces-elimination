      ...
      <div class="col-6 ma-2">
        <table class="table" v-if="tokens.length > 0">
          ...
          <tr :key="k" v-for="(i,k) in tokens">
            <td>{{i.tokenId}}</td>
            <td><img height="500" width="500" :src="i.svg" /></td>
          </tr>
          </tbody>
        </table>
      </div>
...
export default {
  ...
  methods: {
    ...
    async fetch() {
      ...
      for (let x=0; x<number; x++) {
        ...
        let svg = await this.renderContract.methods.image(10000+parseInt(t)).call();

        let buf = new Buffer(svg);

        this.tokens.push({
          tokenId: t,
          svg: 'data:image/svg+xml;base64,'+ buf.toString('base64')
        });
      }
  ...
  async mounted() {
    ...
    this.renderContract = new this.web3.eth.Contract(renderABI, renderAddress);
  }
}
</script>